...
<?php
$id = numer($_GET['id'] ?? 0);
dbSelect("pricing_faqs", "*", "WHERE id = ? LIMIT 1", [$id]);
if ($countrows === 0) {
  sweet("error", "خطأ", "السجل غير موجود", "pricing-faqs");
  exit;
}
$row = $rows[0];
...
if (isset($_POST['submit'])) {
  ...
  dbUpdate("pricing_faqs", $columns, $values, "WHERE id = ? LIMIT 1");
  sweet("success", "تم", "تم التحديث بنجاح", "pricing-faqs");
  exit;
}
?>
<title>تعديل سؤال - صفحة الأسعار</title>
<div class="card mb-4">
  <h5 class="card-header">تعديل سؤال صفحة الأسعار</h5>
