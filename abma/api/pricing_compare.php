...
if (isset($data['action']) and !empty($data['action'])) {
  ...
  if ($action === 'delete') {
    ...
    $id = numer($data['id'] ?? 0);
    dbSelect("pricing_compare_features", "id", "WHERE id=? LIMIT 1", [$id]);
    if ($countrows === 1) {
      dbDelete("pricing_compare_values", "WHERE feature_id=?", [$id]);
      dbDelete("pricing_compare_features", "WHERE id=? LIMIT 1", [$id]);
