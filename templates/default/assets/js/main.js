...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (themeToggle) {
    themeToggle.addEventListener('click', function () {
      ...
      try { localStorage.setItem('default-theme', next); } catch (e) {}
    });
  }
  ...
  /* ---------- Newsletter form — placeholder, not wired to a real provider yet ---------- */
  var newsletterForm = document.getElementById('newsletterForm');
  if (newsletterForm) {
    newsletterForm.addEventListener('submit', function (e) {
      e.preventDefault();
      var note = document.getElementById('newsletterNote');
      if (note) { note.textContent = note.getAttribute('data-success-text') || 'تم الاشتراك!'; }
  ...
  var revealSelectors = [
    ...
    '.blog-card', '.featured-post', '.newsletter', '.stat-item', '.branch-card', '.cert-card'
  ].join(', ');
